    ...
    internal static partial class RedundantSegmentHelper
    {
        internal static bool TryRemoveRedundantSegments(ReadOnlySpan<char> path, ref ValueStringBuilder sb)
        {
            ...
            for (int currPos = rootCharsToSkip; currPos < path.Length; currPos++)
            {
                ...
                bool isSeparator = IsDirectorySeparator(c);
                ...
                if (isSeparator && currPos + 1 < path.Length)
                {
                    ...
                    if (IsNextSegmentOnlyDots(path, currPos, out int totalDots))
                    {
                        ...
                        // Paths larger than 2 are considered valid file or folder names.
                        // Unlinke Windows, they do not need to be removed from the path if they are the last segment.
                        else
                        {
                            currPos += totalDots;
                        }
                ...
                if (!isSeparator || isFullyQualified || sb.Length > rootLength ||
            ...
            if (rootCharsToSkip != rootLength && sb.Length > 0)
            {
                ...
                // e.g. "C:\." => "C:\" or "\\?\C:\.." => "\\?\C:\"
                else if (sb.Length == rootCharsToSkip && path.Length > rootCharsToSkip && IsDirectorySeparator(path[rootCharsToSkip]))
                {

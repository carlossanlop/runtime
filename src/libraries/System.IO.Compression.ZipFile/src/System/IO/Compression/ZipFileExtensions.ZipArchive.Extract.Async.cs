...
public static partial class ZipFileExtensions
{
    ...
    public static Task ExtractToDirectoryAsync(this ZipArchive source, string destinationDirectoryName, CancellationToken cancellationToken) =>
        ExtractToDirectoryAsync(source, destinationDirectoryName, overwriteFiles: false, cancellationToken);

    public static async Task ExtractToDirectoryAsync(this ZipArchive source, string destinationDirectoryName, bool overwriteFiles, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(source);
        ArgumentNullException.ThrowIfNull(destinationDirectoryName);

        foreach (ZipArchiveEntry entry in await source.GetEntriesAsync(cancellationToken).ConfigureAwait(false))
        {

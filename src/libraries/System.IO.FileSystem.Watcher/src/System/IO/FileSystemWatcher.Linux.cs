...
namespace System.IO
{
    ...
    public partial class FileSystemWatcher
    {
        ...
        private static Interop.Sys.NotifyEvents TranslateFilters(NotifyFilters filters)
        {
            ...
            const NotifyFilters filtersForSymlinks =
                NotifyFilters.FollowSymlinks;

            // Unless specified by the user, don't follow symlinks by default
            if ((filters & filtersForSymlinks) == 0)
            {
                result |= Interop.Sys.NotifyEvents.IN_DONT_FOLLOW;
            }
        ...
        private sealed class RunningInstance
        {
            ...
            private void AddDirectoryWatchUnlocked(WatchedDirectory? parent, string directoryName)
            {
                ...
                if ((Interop.Sys.LStat(fullPath, out Interop.Sys.FileStatus status) == 0) &&
                    ((status.Mode & (uint)Interop.Sys.FileTypes.S_IFMT) == Interop.Sys.FileTypes.S_IFLNK))
                {
                    return;
                }

                // Add a watch for the full path.  If the path is already being watched, this will return the existing descriptor.
                // This works even in the case of a rename. We also add EXCL_UNLINK flags to keep parity with Windows where we don't
                // pickup unlinked files (which don't exist in Windows).
                int wd = Interop.Sys.INotifyAddWatch(_inotifyHandle, fullPath, (uint)(_watchFilters | Interop.Sys.NotifyEvents.IN_EXCL_UNLINK));
                if (wd == -1)
                {
